...
check_tool 12 "GNU Make (make)"       "make"       "--version"        "REQ"
check_tool 13 "Taskfile (task)"       "task"       "--version"        "REQ"
check_tool 14 "jq (JSON Processor)"   "jq"         "--version"        "REQ"

echo -e "\n${YELLOW}--- [TERM] TERMINALES, COMUNICACIÓN & EDITORES ---${NC}"
check_tool 15 "Gajim (XMPP)"          "gajim"      "--version"        "OPT"
check_tool 16 "Ghostty Terminal"      "ghostty"    "--version"        "OPT"
check_tool 17 "Zed Editor"            "zed"        "--version"        "OPT"
check_tool 18 "Herdr Multiplexer"     "herdr"      "--version"        "OPT"
check_tool 19 "Neovim"                "nvim"       "--version"        "OPT"
check_tool 20 "VLC Media Player"      "vlc"        "--version"        "OPT"

echo -e "\n${YELLOW}--- [TUI] HERRAMIENTAS TUI Y PRODUCTIVIDAD ---${NC}"
check_tool 21 "fzf (Fuzzy Finder)"    "fzf"        "--version"        "OPT"
check_tool 22 "Lazygit"               "lazygit"    "--version"        "OPT"
check_tool 23 "Yazi (File Manager)"   "yazi"       "--version"        "OPT"
check_tool 24 "Lazydocker"            "lazydocker" "--version"        "OPT"
check_tool 25 "k9s (K8s Monitor)"     "k9s"        "version"          "OPT"

echo -e "\n${YELLOW}--- [EBPF] CONTENEDORES, REDES & EBPF ---${NC}"
check_tool 26 "nerdctl (containerd)"  "nerdctl"    "version"          "OPT"
check_tool 27 "Cilium CLI"            "cilium"     "version --client" "OPT"
check_tool 28 "Hubble CLI"            "hubble"     "version"          "OPT"
check_tool 29 "Trivy (Security)"      "trivy"      "--version"        "OPT"

echo -e "\n${YELLOW}--- [LANG] LENGUAJES & RUNTIMES ---${NC}"
check_tool 30 "Go (Golang)"           "go"         "version"          "OPT"
check_tool 31 "Python 3"              "python3"    "--version"        "OPT"
check_tool 32 "Rust (rustc/cargo)"    "rustc"      "--version"        "OPT"

echo -e "\n${YELLOW}--- [AI] AGENTES IA EN TERMINAL ---${NC}"
check_tool 33 "Claude Code CLI"       "claude"     "--version"        "OPT"
check_tool 34 "Shell-GPT (sgpt)"      "sgpt"       "--version"        "OPT"
check_tool 35 "OMP (Oh My Pi)"        "omp"        "--version"        "OPT"
check_tool 36 "HolmesGPT CLI (AIOps)" "holmes"     "version"          "OPT"
