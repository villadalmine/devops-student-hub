...
.DESCRIPTION
    ...
    para evitar reinstalarlas y ofrece opciones de instalacion por categorias o seleccion:
      1) Solo Software Base DevOps (Core Esencial) (Zoom, Git, GitHub CLI (gh), VS Code, Docker, Terraform, AWS CLI, Azure CLI, Kubectl, Helm, Minikube, GNU Make, Taskfile, jq).
      2) Ecosistema Completo (Obligatorio + Todas las herramientas optativas).
      3) Seleccion Personalizada por Categorias o Numeros (ej: 'BASE', 'AI', 'TERM', 'TUI', 'EBPF', 'LANG', '1-36').
...
# 3. CATALOGO COMPLETO DE HERRAMIENTAS CATEGORIZADAS (36 TOOLS)
$allTools = @(
    ...
    @{ IdNum=12; Categoria="BASE"; CatNombre="Software Base DevOps (Core Esencial)"; Name="GNU Make"; Id="ezwinports.make"; Command="make"; VersionArg="--version"; Description="Orquestador de tareas y automatizacion con Makefiles"; InstallerType="winget"; Tipo="Obligatorio"; CustomCheck={ (Get-Command "make" -ErrorAction SilentlyContinue) -or (Test-Path "C:\Program Files (x86)\GnuWin32\bin\make.exe") -or (Test-Path "$env:LOCALAPPDATA\Microsoft\WinGet\Packages\ezwinports.make_Microsoft.Winget.Source_8wekyb3d8bbwe\bin\make.exe") } },
    @{ IdNum=13; Categoria="BASE"; CatNombre="Software Base DevOps (Core Esencial)"; Name="Taskfile (Task CLI)"; Id="Task.Task"; Command="task"; VersionArg="--version"; Description="Orquestador moderno de tareas basado en Taskfile.yml (alternativa a Make)"; InstallerType="winget"; Tipo="Obligatorio"; CustomCheck={ (Get-Command "task" -ErrorAction SilentlyContinue) -or (Test-Path "$env:LOCALAPPDATA\Microsoft\WinGet\Links\task.exe") } },
    @{ IdNum=14; Categoria="BASE"; CatNombre="Software Base DevOps (Core Esencial)"; Name="jq (JSON Processor)"; Id="jqlang.jq"; Command="jq"; VersionArg="--version"; Description="Procesador y filtro de JSON para scripts DevOps"; InstallerType="winget"; Tipo="Obligatorio"; CustomCheck={ (Get-Command "jq" -ErrorAction SilentlyContinue) -or (Test-Path "$env:LOCALAPPDATA\Microsoft\WinGet\Links\jq.exe") } },
    ...
    @{ IdNum=16; Categoria="TERM"; CatNombre="Terminales y Editores Modernos"; Name="Ghostty Terminal"; Id="AmanThanvi.winghostty"; Command="ghostty"; VersionArg="--version"; Description="Terminal nativa acelerada por GPU"; InstallerType="winget"; Tipo="Optativo"; CustomCheck={ (Get-Command "ghostty" -ErrorAction SilentlyContinue) -or (Get-Command "ghostly" -ErrorAction SilentlyContinue) -or (Get-Command "winghostty" -ErrorAction SilentlyContinue) -or (Test-Path "$env:LOCALAPPDATA\Programs\ghostty\ghostty.exe", "$env:ProgramFiles\Ghostty\ghostty.exe") } },
    @{ IdNum=17; Categoria="TERM"; CatNombre="Terminales y Editores Modernos"; Name="Zed Editor"; Id="ZedIndustries.Zed"; Command="zed"; VersionArg="--version"; Description="Editor ultra-rapido en Rust con soporte de IA"; InstallerType="winget"; Tipo="Optativo"; CustomCheck={ Test-Path "$env:LOCALAPPDATA\Programs\Zed\Zed.exe", "$env:LOCALAPPDATA\Programs\Zed\bin\zed.exe" } },
    @{ IdNum=18; Categoria="TERM"; CatNombre="Terminales y Editores Modernos"; Name="Herdr (Multiplexer)"; Id="Herdr"; Command="herdr"; VersionArg="--version"; Description="Workspace Manager tematico para terminal"; InstallerType="herdr"; Tipo="Optativo" },
    @{ IdNum=19; Categoria="TERM"; CatNombre="Terminales y Editores Modernos"; Name="Neovim (Modal Editor)"; Id="Neovim.Neovim"; Command="nvim"; VersionArg="--version"; Description="Editor modal extensible"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=20; Categoria="TERM"; CatNombre="Terminales y Editores Modernos"; Name="VLC Media Player"; Id="VideoLAN.VLC"; Command="vlc"; VersionArg="--version"; Description="Reproductor multimedia para streams de video"; InstallerType="winget"; Tipo="Optativo"; CustomCheck={ Test-Path "C:\Program Files\VideoLAN\VLC\vlc.exe", "C:\Program Files (x86)\VideoLAN\VLC\vlc.exe" } },

    # --- [TUI] HERRAMIENTAS TUI Y PRODUCTIVIDAD ---
    @{ IdNum=21; Categoria="TUI"; CatNombre="Herramientas TUI y Productividad"; Name="fzf (Fuzzy Finder)"; Id="junegunn.fzf"; Command="fzf"; VersionArg="--version"; Description="Buscador interactivo difuso para terminal"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=22; Categoria="TUI"; CatNombre="Herramientas TUI y Productividad"; Name="Lazygit (TUI Git)"; Id="JesseDuffield.lazygit"; Command="lazygit"; VersionArg="--version"; Description="Visor visual TUI para Git"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=23; Categoria="TUI"; CatNombre="Herramientas TUI y Productividad"; Name="Yazi (TUI Files)"; Id="sxyazi.yazi"; Command="yazi"; VersionArg="--version"; Description="Explorador de archivos TUI en Rust"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=24; Categoria="TUI"; CatNombre="Herramientas TUI y Productividad"; Name="Lazydocker (TUI Docker)"; Id="JesseDuffield.Lazydocker"; Command="lazydocker"; VersionArg="--version"; Description="Visor TUI para Docker"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=25; Categoria="TUI"; CatNombre="Herramientas TUI y Productividad"; Name="k9s (TUI Kubernetes)"; Id="Derailed.k9s"; Command="k9s"; VersionArg="version"; Description="Monitor interactivo para Kubernetes"; InstallerType="winget"; Tipo="Optativo" },

    # --- [EBPF] CONTENEDORES, REDES, SEGURIDAD & EBPF ---
    @{ IdNum=26; Categoria="EBPF"; CatNombre="Contenedores Alternativos, Redes, Seguridad y eBPF"; Name="nerdctl (containerd CLI)"; Id="containerd.nerdctl"; Command="nerdctl"; VersionArg="version"; Description="CLI compatible con Docker para containerd"; InstallerType="nerdctl"; Tipo="Optativo"; CustomCheck={ (Get-Command "nerdctl" -ErrorAction SilentlyContinue) -or (Get-Command "nerctl" -ErrorAction SilentlyContinue) } },
    @{ IdNum=27; Categoria="EBPF"; CatNombre="Contenedores Alternativos, Redes, Seguridad y eBPF"; Name="Cilium CLI (eBPF)"; Id="Cilium.CiliumCLI"; Command="cilium"; VersionArg="version --client"; Description="CLI oficial de Cilium para redes y eBPF en K8s"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=28; Categoria="EBPF"; CatNombre="Contenedores Alternativos, Redes, Seguridad y eBPF"; Name="Hubble CLI (eBPF)"; Id="Cilium.Hubble"; Command="hubble"; VersionArg="version"; Description="CLI de Hubble para observabilidad de red con eBPF"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=29; Categoria="EBPF"; CatNombre="Contenedores Alternativos, Redes, Seguridad y eBPF"; Name="Trivy (Vulnerability Scanner)"; Id="AquaSecurity.Trivy"; Command="trivy"; VersionArg="--version"; Description="Escaner de vulnerabilidades y seguridad para contenedores, K8s e IaC"; InstallerType="winget"; Tipo="Optativo" },

    # --- [LANG] LENGUAJES & RUNTIMES DEVOPS ---
    @{ IdNum=30; Categoria="LANG"; CatNombre="Lenguajes de Programacion y Runtimes"; Name="Go (Golang)"; Id="GoLang.Go"; Command="go"; VersionArg="version"; Description="Lenguaje Go para herramientas K8s, Docker y eBPF"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=31; Categoria="LANG"; CatNombre="Lenguajes de Programacion y Runtimes"; Name="Python 3.12"; Id="Python.Python.3.12"; Command="python"; VersionArg="--version"; Description="Lenguaje Python para scripting y automatizacion DevOps"; InstallerType="winget"; Tipo="Optativo" },
    @{ IdNum=32; Categoria="LANG"; CatNombre="Lenguajes de Programacion y Runtimes"; Name="Rust (Rustup / Cargo)"; Id="Rustlang.Rustup"; Command="rustc"; VersionArg="--version"; Description="Lenguaje de sistemas moderno para herramientas Cloud y CLI de alto rendimiento"; InstallerType="winget"; Tipo="Optativo" },

    # --- [AI] SUITE DE AGENTES DE INTELIGENCIA ARTIFICIAL ---
    @{ IdNum=33; Categoria="AI"; CatNombre="Suite de Agentes de Inteligencia Artificial"; Name="Claude Code CLI"; Id="@anthropic-ai/claude-code"; Command="claude"; VersionArg="--version"; Description="Agente autonomo de Anthropic en terminal"; InstallerType="npm"; Tipo="Optativo" },
    @{ IdNum=34; Categoria="AI"; CatNombre="Suite de Agentes de Inteligencia Artificial"; Name="OpenAI GPT CLI (Shell-GPT)"; Id="shell-gpt"; Command="sgpt"; VersionArg="--version"; Description="CLI para consultas a OpenAI"; InstallerType="pip"; Tipo="Optativo" },
    @{ IdNum=35; Categoria="AI"; CatNombre="Suite de Agentes de Inteligencia Artificial"; Name="OMP (Oh My Pi AI Agent)"; Id="OMP"; Command="omp"; VersionArg="--version"; Description="Agente de IA para terminal (omp.sh)"; InstallerType="omp"; Tipo="Optativo" },
    @{ IdNum=36; Categoria="AI"; CatNombre="Suite de Agentes de Inteligencia Artificial"; Name="HolmesGPT CLI (AIOps)"; Id="holmesgpt"; Command="holmes"; VersionArg="version"; Description="Agente de IA para investigacion de incidentes y alertas en Kubernetes"; InstallerType="pip"; Tipo="Optativo" }
)
